...
def getAffineTransPlugin(k,b):

    for c in trt.get_plugin_registry().plugin_creator_list:
        if c.name == 'AffineTrans':
            parameterList = []
            parameterList.append(trt.PluginField("k", np.float32(k), trt.PluginFieldType.FLOAT32))
            parameterList.append(trt.PluginField("b", np.float32(b), trt.PluginFieldType.FLOAT32))
...
def run(shape, k, b):
    ...
    if os.path.isfile(trtFile):
        ...
        pluginLayer = network.add_plugin_v2([inputT0], getAffineTransPlugin(k,b))
