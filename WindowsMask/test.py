...
def addAffineCPU(inputH, k, b):
    return [inputH[0] * k + b]

def getWindowsMaskPlugin(k,b):

    for c in trt.get_plugin_registry().plugin_creator_list:
        if c.name == 'WindowsMask':
            parameterList = []
            parameterList.append(trt.PluginField("window_size", np.int32(k), trt.PluginFieldType.INT32))
            parameterList.append(trt.PluginField("shift_size", np.int32(b), trt.PluginFieldType.INT32))
...
def run(shape, k, b):
    testCase = "<shape%s,k&b=%f,%f>" % (shape, k ,b)
    ...
    if os.path.isfile(trtFile):
        ...
        inputT0 = network.add_input('inputT0', trt.DataType.FLOAT,[-1,shape[1],shape[2]])
        inputT1 = network.add_input('inputT1', trt.DataType.INT32,[-1,1,1])

        profile.set_shape(inputT0.name, [1,shape[1],shape[2]], [4,shape[1],shape[2]], [16,shape[1],shape[2]])
        profile.set_shape(inputT1.name, [1,1,1], [4,1,1],[16,1,1])

        config.add_optimization_profile(profile)

        pluginLayer = network.add_plugin_v2([inputT0,inputT1], getWindowsMaskPlugin(k,b))
    ...
    bufferH = []
    # ori_data = np.arange(np.prod(shape), dtype=np.float16).reshape(shape)
    ...
    outputCPU = addAffineCPU(bufferH[:nInput], k, b)
